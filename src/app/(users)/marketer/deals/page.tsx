...
function invoiceBadge(status: string) {
  ...
  if (lower.includes("not sent")) return "bg-[#FEF2F2] text-[#B42318]";
  return "bg-[#ECFDF3] text-[#027A48]";
}

function paymentBadge(status: string) {
  const lower = status.toLowerCase();
  if (lower.includes("not received")) return "bg-[#FEF2F2] text-[#B42318]";
  return "bg-[#ECFDF3] text-[#027A48]";
}
                ...
                >
                  ...
                  <thead>
                    <tr className="bg-[#F9FAFB] border-b border-[#EAECF0]">
                      <th className="text-left px-4 py-4 text-[#667085] text-xs font-bold uppercase tracking-wider">
                        #
                      </th>
                      <th className="text-left px-4 py-4 text-[#667085] text-xs font-bold uppercase tracking-wider">
                        ENQUIRY
                      ...
                      <th className="text-center px-4 py-4 text-[#667085] text-xs font-bold uppercase tracking-wider">
                        ACTIONS
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#EAECF0]">
                    {paginated.length === 0 ? (
                      <tr>
                        <td
                          className="px-4 py-8 text-sm text-[#667085]"
                          colSpan={6}
                        >
                          No deals found.
                        </td>
                      ...
                      paginated.map((d, i) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-5 text-sm font-semibold text-[#101828] underline truncate">
                            <button
                              onClick={() => openEnquiry({
                                ...
                                vtmStatus: "", adminStatus: ""
                              } as Enquiry)}
                              className="text-[#101828] hover:text-primary hover:underline text-left truncate w-full"
                            >
                              {d.enquiry}
                            </button>
                          </td>
                          ...
                          <td className="px-4 py-5">
                            <span
                              className={`px-2.5 py-1.5 rounded-full text-xs font-bold ${invoiceBadge(d.invoiceSent)}`}
                            >
                              {d.invoiceSent}
                            </span>
                          </td>
                          <td className="px-4 py-5">
                            <span
                              className={`px-2.5 py-1.5 rounded-full text-xs font-bold ${paymentBadge(d.paymentReceived)}`}
                            >
                              {d.paymentReceived}
                            </span>
                          </td>
                          <td className="px-4 py-5">
                            <div className="flex items-center justify-center text-[#667085]">
                              <button
                                onClick={() => handleAction(d.id, "Manage")}
