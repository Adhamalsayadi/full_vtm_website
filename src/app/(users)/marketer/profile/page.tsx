        ...
        <main className="flex-1 p-6 md:p-10 overflow-auto">
          <div className="max-w-3xl mx-auto">
            ...
            <form
              ...
              className="bg-white rounded-2xl border border-[#EAECF0] shadow-sm p-8 space-y-7"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {[
                  ...
                  { label: "Position", name: "position", disabled: true },
                  ...
                  <div key={field.name}>
                    <label className="block text-xs font-bold text-[#344054] uppercase tracking-wider mb-1.5">
                      {field.label}
                      {field.disabled && (
                        <span className="text-[#98A2B3] text-xs font-normal">
                          {" "}
                          (read-only)
                        </span>
                      )}
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-2.5 bg-[#F9FAFB] border border-[#EAECF0] rounded-xl outline-none focus:ring-2 focus:ring-primary/20 text-sm transition-all ${field.disabled ? "opacity-60 cursor-not-allowed" : ""}`}
                    />
                  </div>
                ))}
              </div>

              <div className="text-xs text-[#667085] flex gap-2 items-center bg-[#EFF8FF] p-3 rounded-lg border border-[#D1E9FF]">
                <span className="w-1.5 h-1.5 rounded-full bg-[#175CD3] block shrink-0"></span>
                Changes to your Email or Phone number require VT Admin approval. Submitting will send a request.
              </div>
              ...
              <div className="flex gap-3 pt-2 border-t border-[#F2F4F7]">
                ...
                >
                  <Save size={16} />
                  {requested ? "Request Sent!" : "Request Change"}
                </button>
              </div>
            </form>
          </div>
